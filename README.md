...
`frontend apps list-replay-sessions` includes `list_meta` in JSON output when its event scan may be incomplete. `frontend sessions get-replay` saves every recording for a session in one file.
