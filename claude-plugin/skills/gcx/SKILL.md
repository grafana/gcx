...
- `gcx datasources list` / `gcx datasources get` — discover available datasources (`gcx datasources get <uid> -o yaml` emits an apply-ready manifest)
- `gcx datasources prometheus` — PromQL queries (query, labels, metadata, targets)
- `gcx datasources loki` — LogQL queries (query, labels, series)
- `gcx datasources pyroscope` — profiling queries
- `gcx datasources tempo` — trace queries
- `gcx datasources query <uid> <expr>` — auto-detect datasource type
...
- `delete UID...` — prompts unless `--force`/`--yes` or `GCX_AUTO_APPROVE`. Agent
  mode does **not** auto-approve: without one of those it fails with an
  actionable error. Batch-safe with partial-failure exit code 4.
