...
| **Private probe infra** | Only private probes failing; public probes healthy; no pattern across regions | Private probe agent down or network-isolated; agent version mismatch; proxy/firewall between agent and target | Check private probe agent status (`grafanactl synth probes list`); verify agent connectivity; check agent logs |
