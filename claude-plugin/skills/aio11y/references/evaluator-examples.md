...
Copy-paste definitions for `evaluators create -f`, one per kind, with the
...
- `version` is required at the top level. It versions the evaluator itself —
  ...
  schema. To change an evaluator later, re-run `create` with the same
  `evaluator_id` and a new `version`; re-using an existing version is a 409.
- Test any evaluator without persisting results:
  `gcx aio11y evaluators test -e <evaluator-id> -g <generation-id>`
...
Browse `gcx aio11y templates list` / `templates get <id> -o yaml` for the
...
gcx aio11y judge models --provider <provider-id>
```
...
config:
  provider: <provider-id>   # from `judge providers`
  model: <model-id>         # from `judge models --provider <provider-id>`
