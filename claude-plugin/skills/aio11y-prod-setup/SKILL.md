---
name: aio11y-prod-setup
description: >
  Sets up production evaluation and guardrails for a DEPLOYED AI agent in Grafana
  AI Observability, grounded in the agent's own code and its real ingested traffic.
  The judgment layer on top of the `aio11y` skill: it reads the agent's source
  ...
  `gcx aio11y`. New guards start disabled + warn. It DOES create tenant-level objects —
...
# AI Observability — production evals & guards setup

The production counterpart to `aio11y-eval-starter` (which runs pre-ship, on code alone,
...
## What this skill does that `aio11y` doesn't

The sibling `aio11y` skill is the **mechanics** layer: exact CLI flags, evaluator/rule YAML
...
Two gaps this skill fills beyond `aio11y`:

- **Recommendation from evidence** — `aio11y` starts once you know what to create; this decides.
- **Guards** — `aio11y` documents evaluators and rules but not guards (hook-rules), even though
  `gcx aio11y guards` exists. This skill carries the guard shapes
...
the `aio11y` skill and to `gcx aio11y <sub> --help` rather than restating it here.

## Rules

- Every connection to the stack goes through **`gcx aio11y`** — never raw HTTP, never a hand-held
...
- **Check before recommending.** Always list what already exists first
  (`gcx aio11y evaluators list`, `rules list`, `guards list`) and never recommend a duplicate.
...
- This skill **does** create tenant-level objects — that is its job, the one thing that separates
  it from `aio11y-eval-starter`. But every creation is **explicit and confirmed**: show the exact
  YAML, get a yes, then create it with the matching `gcx aio11y` command. A yes for one object
...
- Prefer **starting from an evaluator template** (`gcx aio11y templates list`, then
  `gcx aio11y templates get`) over authoring
...
- If a required input is missing (the agent's name as AI Observability sees it, or `gcx` auth),
  ask — don't guess.
...
**Code** (as `aio11y-eval-starter` Step 1). Find and record file:line for: the entrypoint, the
...
1. Find the agent as AI Observability sees it: `gcx aio11y agents list` (and `agents get` /
...
2. Sample recent conversations: `gcx aio11y conversations search --filters 'agent = "<name>"'`
   (add `status = "error"`, time windows, `tool.name`, `eval.passed = false`) and
   `gcx aio11y generations get <id>` for detail. Look for long tool loops, over-refusals, PII
   ...
   - **Some agents have generations but no conversations** (e.g. single-shot agents whose spans
     ...
     (`gcx aio11y generations get <id>`) and use `selector: all_assistant_generations` for rules
...
agent has essentially no traffic, stop — this is the wrong skill; `aio11y-eval-starter` (offline
suite) is the right one until traffic exists.

## Step 2 — Inventory what already exists

Before proposing anything: `gcx aio11y evaluators list`, `gcx aio11y rules list`,
`gcx aio11y guards list` (add `-o yaml` to see full definitions). For each concern you're about to
...
version, so always resolve the current set with `gcx aio11y templates list` before using a name;
...
kinds, templates), the `aio11y` skill is the reference — don't restate it here.
...
their source of truth after apply is the stack, not the repo. Add `aio11y-prod/` to `.gitignore`
...
you'll pass to `gcx aio11y <kind> create -f`. Use the **top-level-fields** YAML shape that the
...
**Rules and evaluators**: follow the `aio11y` skill's input format exactly. Start an evaluator
from a template (`gcx aio11y templates get <id> -o yaml`), give it your own `evaluator_id`, and
...
**Guards** — the shape the `aio11y` skill omits (`gcx aio11y guards create -f guard.yaml`; the
resource `Kind` is `HookRule`).

> **Hard gate: do not draft a guard until you have captured the exact accepted create-file shape
> from the current `gcx` version.** Run `gcx aio11y guards create --help` and inspect a real
> definition (`gcx aio11y guards get <id> -o yaml`, or `guards list -o yaml`); if none exists, get
> the schema from `--help` / the resource definition. Only then write a guard file.

This skill (not the `aio11y` skill) carries the guard shape, and field names/nesting can drift by
...
```yaml
# ILLUSTRATIVE — confirm fields with `gcx aio11y guards create --help` before use.
# PROD-SETUP DRAFT — creates a TENANT-LEVEL guard (HookRule) via gcx aio11y guards create.
...
1. **For an `llm_judge` evaluator, tune the prompt before creating it — this is the real work,
   ...
   (`gcx aio11y generations get <id>`), run the draft config with
   `gcx aio11y evaluators test -f <request>.yaml -g <gen-id>` (tests without persisting), and
...
3. **Apply** via gcx, only after the yes: `gcx aio11y evaluators create -f evaluators/<id>.yaml`,
   then `gcx aio11y rules create -f rules/<id>.yaml`, then
   `gcx aio11y guards create -f guards/<id>.yaml`. Evaluators are create-or-update (same id
   ...
   - **Verify each created guard came back `enabled: false` and `action_on_fail: warn`.** The
     create output (or `gcx aio11y guards get <id> -o yaml`) echoes the stored object — check it.
...
3. The follow-through the developer still owns:
   - **Guards:** watch the `warn` guards on real traffic; flip to `deny` + `enabled` only once the
     false-positive rate looks acceptable — `gcx aio11y guards update <id> -f ...`. That flip is
     theirs to make, not this skill's.
   - **Rules:** raise `sample_rate` once scores look sane and cost is understood
     (`gcx aio11y rules update`); add alerting on regressions if wanted.
   - Inspect everything in AI Observability (rules/guards/evaluators pages, the conversation
     Quality view) or via the `gcx aio11y` list and get commands.
4. A one-line pointer back: for pre-ship offline evaluation of a new agent or version,
   `aio11y-eval-starter` is the counterpart; for control-plane mechanics, the `aio11y` skill.
