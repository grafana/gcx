...
- New `gcx signup` creates a free Grafana Cloud account in the browser and connects gcx to its first stack: gcx opens the sign-up page, waits while you create the account, verify your email, and create the stack, and saves the connection when you approve "Connect gcx". It asks no questions, so agents can run it too; a person completes the browser steps. It only ever saves a new connection, refusing an existing context, stack entry, or destination environment variable before the browser opens, and if it fails once the browser step has started, the error says how to finish with `gcx login` instead of signing up again.
...
- The browser page shown after approving a gcx login now says "Authorization complete" and points back to the terminal instead of claiming the connection is saved: gcx still validates and saves the connection afterwards, and reports the result there. A rate limit or service error while finishing the login now says to wait or retry `gcx login`.
