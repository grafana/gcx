...
// runAgento11y executes the real root command tree with only the Agent
// Observability provider mounted. A fresh tree per invocation keeps cobra
// flag state from leaking between cases.
func runAgento11y(t *testing.T, args ...string) (string, string, error) {
	t.Helper()
...
// TestAgento11y_ExplicitConfigWinsOverDefault is the core #951 regression: a
...
func TestAgento11y_ExplicitConfigWinsOverDefault(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			recA.reset()
			recB.reset()
			_, stderr, err := runAgento11y(t, tc.args...)
...
// TestAgento11y_ExplicitConfigWinsOverGCXConfigEnv: GCX_CONFIG points at server A,
...
func TestAgento11y_ExplicitConfigWinsOverGCXConfigEnv(t *testing.T) {
	...
	t.Run("positive control: GCX_CONFIG alone routes to A", func(t *testing.T) {
		recA.reset()
		recB.reset()
		_, stderr, err := runAgento11y(t, "agento11y", "evaluators", "list", "-o", "json")
		require.NoError(t, err, "stderr: %s", stderr)
...
// TestAgento11y_ExplicitConfigWithoutDefault reproduces the original #951 error
...
func TestAgento11y_ExplicitConfigWithoutDefault(t *testing.T) {
	testutils.SandboxConfigEnv(t)
	srvB, recB := newPluginServer(t)
	cfgB := writeConfigFile(t, filepath.Join(t.TempDir(), "b.yaml"), srvB.URL, "token-b", 22222)
...
// TestAgento11y_CollectionsUpdateEnvelopeNamespace covers the mixed-path defect
...
func TestAgento11y_CollectionsUpdateEnvelopeNamespace(t *testing.T) {
	home := testutils.SandboxConfigEnv(t)
	srvA, recA := newPluginServer(t)
	srvB, recB := newPluginServer(t)
	writeConfigFile(t, defaultConfigPath(home), srvA.URL, "token-a", 11111)
	...
	stdout, stderr, err := runAgento11y(t, "agento11y", "collections", "update", "c-1", "--name", "renamed", "-o", "json", "--config", cfgB)
	require.NoError(t, err, "stderr: %s", stderr)
...
// TestAgento11y_ContextFlagSmoke guards what #1012 fixed: --context selects
// between contexts of the discovered default config, credentials included.
func TestAgento11y_ContextFlagSmoke(t *testing.T) {
	home := testutils.SandboxConfigEnv(t)
	srvA, recA := newPluginServer(t)
	srvB, recB := newPluginServer(t)
...
`, srvA.URL, srvB.URL)
	...
	_, stderr, err := runAgento11y(t, "--context", "b", "agento11y", "evaluators", "list", "-o", "json")
	require.NoError(t, err, "stderr: %s", stderr)
...
// TestAgento11y_EnvVarSmoke guards the env-override path that always worked for
...
func TestAgento11y_EnvVarSmoke(t *testing.T) {
	home := testutils.SandboxConfigEnv(t)
	srvA, recA := newPluginServer(t)
	srvB, recB := newPluginServer(t)
	writeConfigFile(t, defaultConfigPath(home), srvA.URL, "token-a", 11111)
