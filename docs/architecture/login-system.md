...
The pipeline reads top-to-bottom in `Run()` (login.go:333). Each step returns
early on failure; sentinel branches unwind to the CLI for interactive
resolution and re-entry:

1. **Server check** (login.go:335). Missing server → `ErrNeedInput{server}`.
2. **Scheme normalization and portal check** (login.go:345). A bare hostname is
...
3. **Target detection** (login.go:359). Delegates to `detectTarget`; see the
...
4. **Grafana auth resolution** (`resolveGrafanaAuth`, login.go:669). Picks
   between explicit token and OAuth based on input flags. Missing auth
   yields `ErrNeedInput{grafana-auth}`.
5. **Context-name derivation** (login.go:401). Falls back to
...
7. **Validation** (login.go:445). Delegated to `Validate` (see below); the
   CLI offers an escape hatch for interactive users when validation fails.
8. **Persistence** (`persistContext`, login.go:931). Writes only after all
