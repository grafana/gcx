...
│   ├── credentials/          # OS-keychain backend (zalando/go-keyring) for token-shaped secrets; sentinel format + Store interface; classifies a backend failure as unavailable (plaintext fallback), locked (fatal), or unknown (fatal); auto-disabled under `go test`
