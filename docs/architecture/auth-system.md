...
hole. The state check also decides ownership: only a `GET` that carries this
attempt's state may consume the single-use handler. Other requests get a notice
page or `405` and leave the login waiting, because every local process can
reach the loopback listener and a tab from an earlier attempt lands on the same
port when a new attempt picks it again. A callback with this attempt's state
that fails a check before the token exchange (no code, or, in the stack flow, a
missing or untrusted `endpoint`) spent nothing, so it also leaves the login
waiting, as the paste route re-prompts. Only a callback that reaches the
exchange, or carries a provider `error`, consumes the handler. The terminal
still says when a callback was ignored, because an incomplete callback from the
consent page itself would otherwise leave the login waiting with no reason. A consent-page Cancel
(`error=user_cancelled`) ends the flow as `ErrBrowserCancelled`, which the
login command reports with exit code 5. The `endpoint` query parameter — supplied by the assistant plugin and
...
Without a stack URL, the flow opens the Grafana Cloud stack launcher
(`<portal>/launch/a/grafana-assistant-app/cli/auth?…`), which forwards the same
