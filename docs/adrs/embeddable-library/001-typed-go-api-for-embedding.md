...
- Keep an eye on whether the dependency-version collisions hit during the SLO pilot (`grafana-openapi-client-go`, `go.opentelemetry.io/otel`, `prometheus/alertmanager`) recur as more products get promoted. If pinning a growing pile of `replace` directives in every consumer's `go.mod` turns into a standing cost rather than a one-off, that's the concrete signal to revisit "same module as the CLI" and consider a separate, leaner module after all — the option this ADR left open rather than ruled out.
