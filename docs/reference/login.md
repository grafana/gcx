...
12. **A credential was `rejected before network use`**
    - *Means:* a keychain reference was missing/foreign, a destination changed, the OS keychain was locked, or an environment credential was paired with an auto-discovered repository destination. gcx withheld it instead of sending an empty or misrouted credential. If the reason says `the OS keychain is locked`, see entry 15 below.
...
15. **`Keychain locked`**
    ...
    - *Fix:* Unlock the keychain, then run the command again. On Linux, unlock the keyring with `systemd-ask-password 'Keyring password: ' | tr -d '\n' | gnome-keyring-daemon --replace --daemonize --unlock`, or run gcx from a desktop session that can show a password prompt. You must pipe the password in. `gnome-keyring-daemon --unlock` reads the password from standard input, and it shows no prompt. `--daemonize` makes the process fork, so a password that you type does not reach the child process. The daemon then waits for input, and the collection stays locked. The password must also carry no trailing newline, which is the reason for the `tr` filter. In bash you can prompt with `read -rsp 'Keyring password: ' PW`, and in zsh with `read -rs "PW?Keyring password: "`, then pipe `printf '%s' "$PW"` into the daemon. To read the lock state, run `busctl --user get-property org.freedesktop.secrets /org/freedesktop/secrets/collection/login org.freedesktop.Secret.Collection Locked`. On macOS and Windows, unlock the keychain through the OS. For a CI job that has no keychain at all, supply the credential through an environment variable such as `GRAFANA_TOKEN`.
