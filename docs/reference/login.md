...
15. **`Keychain locked`**
    ...
    - *Fix:* Unlock the keychain, then run the command again. On Linux, unlock the keyring with `gnome-keyring-daemon --replace --daemonize --unlock`, or run gcx from a desktop session that can show a password prompt. This command reads the password from standard input, so run it from an interactive terminal. A non-interactive shell supplies no password, and the collection stays locked. To read the lock state, run `busctl --user get-property org.freedesktop.secrets /org/freedesktop/secrets/collection/login org.freedesktop.Secret.Collection Locked`. On macOS and Windows, unlock the keychain through the OS. For a CI job that has no keychain at all, supply the credential through an environment variable such as `GRAFANA_TOKEN`.
