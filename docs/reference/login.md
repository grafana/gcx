...
15. **`Keychain locked`**
    ...
    - *Fix:* Unlock the keychain, then run the command again. On Linux, unlock the keyring, or run gcx from a desktop session that can show a password prompt. The unlock command depends on the session, so follow the procedure in [Unlock a GNOME keyring in a headless session](../sources/configuration.md#unlock-a-gnome-keyring-in-a-headless-session). Three constraints make a one-line command unreliable: the daemon reads the password from standard input with no prompt, `--daemonize` makes the process fork so a typed password never reaches it, and a trailing newline leaves the collection locked without an error. A service manager that runs the daemon can also hold the `org.freedesktop.secrets` name and refuse to yield it to `--replace`. To read the lock state, run `busctl --user get-property org.freedesktop.secrets /org/freedesktop/secrets/collection/login org.freedesktop.Secret.Collection Locked`. On macOS and Windows, unlock the keychain through the OS. For a CI job that has no keychain at all, supply the credential through an environment variable such as `GRAFANA_TOKEN`.
