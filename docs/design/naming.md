...
|--------|-----------|-------|
| Entity | `list`, `get`, `create`, `update`, `delete`, `upsert`, `patch` (reserved) | `get` = one addressable subject or singleton; `update` = typed resource update (may be partial); `upsert` = direct create-or-update of one or more explicitly supplied subjects, processed independently and possibly non-atomically (the caller never chooses create vs update; workflow, not cardinality, distinguishes it from `push`); `patch` reserved for explicit patch-document/patch-operation surfaces |
...
| Query | `query`, `search` + approved shorthands `labels`, `series`, `metrics`, `metadata` | shorthands map to real list/query semantics; the shorthand set is closed. `profile-types` is **not** a shorthand — it is renamed to `list-profile-types` at both of its mount points (decided 2026-07-17, respelled in review 2026-07-21, ADR §4). Factual caveat: several datasource reads persist discovered datasource UIDs today — docs, help text, and classification must say so (ADR §4; any refactor is separate out-of-scope work) |
