...
```bash
grafanactl slo definitions get yfxvco8oud0wknbcqs0e8 -o json
```
...
```bash
# Visual overview
grafanactl query -d grafanacloud-prom \
...
```bash
grafanactl alert rules list -o json | jq '[.[] | select(.name | test("Git Sync Provisioning.*Error"; "i"))]'
```
...
## grafanactl Gaps / Improvement Suggestions

1. **`grafanactl slo` needs a status/report command per SLO**: `grafanactl slo reports list` didn't allow filtering by SLO UID easily. A `grafanactl slo status <uid>` that shows current burn rate, error budget remaining, and which clusters are breaching would shortcut the entire investigation.

2. **SLO → alert rule mapping is missing**: There's no way to go from an SLO UID to its generated alert rules directly. Had to search alert rules by group name pattern matching. A `grafanactl slo alerts <uid>` command would help.

3. **`grafanactl slo definitions get` should include current state**: The definition output has no runtime info — no current error budget, no burn rate, no firing status. Adding a `--with-status` flag or a separate `grafanactl slo status` command would eliminate the need for manual PromQL queries.
...
5. **Query flag naming**: `grafanactl query` uses `--from`/`--to` (not `--start`/`--end`). This is fine but worth noting since Grafana UI and some APIs use different naming.
