...
func (opts *Options) Encode(dst io.Writer, value any) error {
	...
	// Suppress the hint when --jq is in use — the user already has a more
	// powerful transformation in hand.
	isJSONLike := codec.Format() == format.JSON || codec.Format() == agentsFormat
	if !opts.agentHintShown && agent.IsAgentMode() && isJSONLike && len(opts.JSONFields) == 0 && !opts.JSONDiscovery && opts.jqQuery == nil {
