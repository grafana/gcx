...
func evaluateChecks(report *ExperimentReport, spec checkSpec) CheckResult {
	...
	if normalizeStatus(run.Status) != statusCompleted {
		result.Checks = append(result.Checks, Check{
			Name:     checkNameExperimentStatus,
			Verdict:  CheckVerdictFail,
			Observed: fmt.Sprintf("experiment status is %q, not %q", run.Status, statusCompleted),
		})
