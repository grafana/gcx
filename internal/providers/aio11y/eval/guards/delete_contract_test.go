package guards //nolint:testpackage // Tests drive the unexported runDelete seam and deleteOpts wiring.

// Contract tests for `gcx agento11y guards delete` (#387 agent output
...
// strings (success says "guard", the error prefix says "hook rule", exactly
// as before), the gcx.mutation_batch document in agent mode, the
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				// The pre-migration error prefix said "hook rule".
				assert.Contains(t, err.Error(), "deleting hook rule g-2: boom")
			} else {
