...
// FormatText is the output format name for replay text codecs.
...
func (c *ReplayRecordingTableCodec) Format() format.Format { return FormatText }
...
func (c *ReplayRecordingTableCodec) Decode(_ io.Reader, _ any) error {
...
func resolveFirstRecordingID(ctx context.Context, client *Client, appID, sessionID string) (string, error) {
	...
	if len(resp.Items) == 0 {
		return "", errors.New("no replay recordings found for session")
	}
...
func newInspectReplaySessionCommand(loader RESTConfigLoader) *cobra.Command {
  ...
  gcx frontend apps inspect-replay-session my-web-app-42 abc-session-123 --limit 0`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			rows := make([]ReplayRecordingRow, len(resp.Items))
...
// ---------------------------------------------------------------------------
// inspect-replay-segment command
// ---------------------------------------------------------------------------

type inspectReplaySegmentOpts struct {
	IO          cmdio.Options
	Save        string
	RecordingID string
}
...
func (o *inspectReplaySegmentOpts) Validate() error {
	if err := o.IO.Validate(); err != nil {
		return err
	}
	return nil
...
func newInspectReplaySegmentCommand(loader RESTConfigLoader) *cobra.Command {
	opts := &inspectReplaySegmentOpts{}
	cmd := &cobra.Command{
		Use:   "inspect-replay-segment <app-name> <session-id> <segment-id>",
		Short: "Inspect events for a session replay segment.",
		Example: `  # Show event summary for replay segment 0.
  ...
  gcx frontend apps inspect-replay-segment my-web-app-42 abc-session-123 0 -o json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.IO.OutputFormat != string(FormatText) {
				return opts.IO.Encode(cmd.OutOrStdout(), segment.Events)
			}
