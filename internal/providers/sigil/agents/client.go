...
// Client is an HTTP client for Sigil agent catalog endpoints.
type Client struct {
...
func (c *Client) List(ctx context.Context, limit int) ([]Agent, error) {
	...
	return sigilhttp.ListAll[Agent](ctx, c.base, "/query/agents", query, limit)
}
...
func (c *Client) Versions(ctx context.Context, name string) ([]AgentVersion, error) {
	query := url.Values{"name": {name}}
	return sigilhttp.ListAll[AgentVersion](ctx, c.base, "/query/agents/versions", query)
}
