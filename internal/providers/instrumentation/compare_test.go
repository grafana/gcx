...
func TestCompareNamespace(t *testing.T) {
	tests := []struct {
		name      string
		remote    *instrumentation.AppSpec
		localNS   *instrumentation.NamespaceConfig
		wantEmpty bool
		wantApps  []instrumentation.RemoteOnlyApp
	}{
		{
			name:      "nil remote returns empty diff",
			remote:    nil,
			localNS:   &instrumentation.NamespaceConfig{Name: "payments"},
			wantEmpty: true,
		},
		{
			name:      "nil localNS returns empty diff",
			remote:    &instrumentation.AppSpec{Namespaces: []instrumentation.NamespaceConfig{{Name: "payments"}}},
			localNS:   nil,
			wantEmpty: true,
		},
		{
			name: "target namespace absent in remote — no remote-only items",
			remote: &instrumentation.AppSpec{
				Namespaces: []instrumentation.NamespaceConfig{
					{Name: "billing", Apps: []instrumentation.AppConfig{{Name: "invoicer"}}},
				},
			},
			localNS:   &instrumentation.NamespaceConfig{Name: "payments"},
			wantEmpty: true,
		...
		{
			name: "other namespace has extra items — not flagged (FR-014)",
			remote: &instrumentation.AppSpec{
				Namespaces: []instrumentation.NamespaceConfig{
					{Name: "payments", Apps: []instrumentation.AppConfig{{Name: "checkout"}}},
					{Name: "billing", Apps: []instrumentation.AppConfig{{Name: "invoicer"}}},
				},
			},
			localNS: &instrumentation.NamespaceConfig{
				Name: "payments",
				Apps: []instrumentation.AppConfig{{Name: "checkout"}},
			},
			wantEmpty: true,
		},
		{
			name: "remote has extra app in target namespace — optimistic-lock fires",
			remote: &instrumentation.AppSpec{
				Namespaces: []instrumentation.NamespaceConfig{
					{Name: "payments", Apps: []instrumentation.AppConfig{{Name: "checkout"}, {Name: "fraud-svc"}}},
					{Name: "billing", Apps: []instrumentation.AppConfig{{Name: "invoicer"}}},
				},
			},
			localNS: &instrumentation.NamespaceConfig{
			...
			wantEmpty: false,
			wantApps: []instrumentation.RemoteOnlyApp{
				{Namespace: "payments", App: "fraud-svc"},
			},
		},
		{
			name: "local superset of remote in target namespace — no conflict",
			remote: &instrumentation.AppSpec{
				Namespaces: []instrumentation.NamespaceConfig{
					{Name: "payments", Apps: []instrumentation.AppConfig{{Name: "checkout"}}},
				},
			...
			localNS: &instrumentation.NamespaceConfig{
				Name: "payments",
				Apps: []instrumentation.AppConfig{{Name: "checkout"}, {Name: "fraud-svc"}},
			},
			wantEmpty: true,
		},
		{
			name: "exact match in target namespace — no conflict",
			remote: &instrumentation.AppSpec{
				Namespaces: []instrumentation.NamespaceConfig{
					{Name: "payments", Apps: []instrumentation.AppConfig{{Name: "checkout"}}},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			diff := instrumentation.CompareNamespace(tt.remote, tt.localNS)
			...
			assert.False(t, diff.IsEmpty(), "expected non-empty diff")
			assert.Empty(t, diff.Namespaces, "CompareNamespace must never produce remote-only namespaces")
			assert.ElementsMatch(t, tt.wantApps, diff.Apps)
		})
	}
}
