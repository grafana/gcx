...
// CompareNamespace evaluates remote-only apps scoped to a single (cluster, namespace) pair.
// Only the named namespace entry is examined; items in other namespaces are ignored (FR-014).
// Cluster comparisons use the full-payload Compare instead (FR-015).
func CompareNamespace(remoteCluster *AppSpec, localNS *NamespaceConfig) *Diff {
	diff := &Diff{}
	if remoteCluster == nil || localNS == nil {
		return diff
	}

	// Find the target namespace in the remote cluster payload.
	var remNS *NamespaceConfig
	for i := range remoteCluster.Namespaces {
		if remoteCluster.Namespaces[i].Name == localNS.Name {
			remNS = &remoteCluster.Namespaces[i]
			break
		}
	}
	if remNS == nil {
		// Target namespace not present remotely — no remote-only items block the write.
		return diff
	}

	// Index local apps.
	localApps := make(map[string]struct{}, len(localNS.Apps))
	for _, app := range localNS.Apps {
		localApps[app.Name] = struct{}{}
	}

	// Find remote apps absent from the local manifest.
	for _, remApp := range remNS.Apps {
		if _, ok := localApps[remApp.Name]; !ok {
			diff.Apps = append(diff.Apps, RemoteOnlyApp{
				Namespace: remNS.Name,
