...
func TestDiscoverCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "missing cluster arg returns error",
			args:       []string{},
			wantErr:    true,
		...
		},
		{
			...
			args:        []string{"empty-cluster"},
			useServer:   true,
			setupStatus: http.StatusOK,
			runStatus:   http.StatusOK,
			runBody:     `{}`,
			wantStderr:  `note: no workloads found in cluster "empty-cluster".`,
		},
		{
			name:        "json output",
			args:        []string{"prod-1", "-o", "json"},
			useServer:   true,
			setupStatus: http.StatusOK,
			runStatus:   http.StatusOK,
			runBody:     `{"items":[{"clusterName":"prod-1","namespace":"default","name":"web-abc","displayName":"web","workloadType":"deployment","instrumentationStatus":"active"}]}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd := discover.NewCommandForTest(loader)
