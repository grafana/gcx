package rules //nolint:testpackage // Tests drive the unexported runDelete seam and deleteOpts wiring.

// Contract tests for `gcx agento11y rules delete` (#387 agent output
...
func TestDelete_ExplicitYAMLOverride(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	require.NoError(t, runDelete(&stdout, &stderr, opts, []string{"r-1"}, func(string) error { return nil }))
