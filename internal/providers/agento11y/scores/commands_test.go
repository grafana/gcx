...
// assertTruncationWarning drives cmd against a server that returns two items
// plus a next_cursor, and asserts the truncation hint lands on stderr only while
// stdout stays valid JSON. Shared by the rule and generation list-scores tests.
func assertTruncationWarning(t *testing.T, cmd *cobra.Command, args []string, envelope bool) {
	t.Helper()
	home := testutils.SandboxConfigEnv(t)
...
func TestNewListRuleScoresCommand_TruncationWarning(t *testing.T) {
	assertTruncationWarning(t, scores.NewListRuleScoresCommand(nil), []string{"rule-1", "--limit", "2", "-o", "json"}, true)
}
...
func TestNewListScoresCommand_ZeroLimitCappedWithRoute(t *testing.T) {
	home := testutils.SandboxConfigEnv(t)
	// Always return a full page + cursor: generation --limit 0 must stop at the
	// hard cap rather than draining unbounded.
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		page := make([]scores.Score, 500)
		for i := range page {
			page[i] = scores.Score{ScoreID: "s", ScoreKey: "a"}
	...
	cmd := scores.NewListScoresCommand(nil)
	var stdout, stderr bytes.Buffer
	cmd.SetOut(&stdout)
	cmd.SetErr(&stderr)
	cmd.SetArgs([]string{"gen-1", "--limit", "0", "-o", "json"})
	require.NoError(t, cmd.ExecuteContext(context.Background()), "stderr: %s", stderr.String())

	// The fetch is bounded by the hard cap, but a generation has no filters, so
	// the hint offers a larger --limit as the route to more rows (never the
	// filters-based "refine" advice, which generation can't act on).
	assert.Contains(t, stderr.String(), "showing first")
	assert.Contains(t, stderr.String(), "more results are available")
	assert.Contains(t, stderr.String(), "--limit")
	assert.NotContains(t, stderr.String(), "Refine filters")
	var got []scores.Score
	require.NoError(t, json.Unmarshal(stdout.Bytes(), &got))
	assert.Len(t, got, scores.ScoreHardCap(), "generation fetch is bounded by the hard cap")
}

func TestListScoresCommands_RejectEmptyID(t *testing.T) {
	cases := []struct {
		name string
		cmd  *cobra.Command
		want string
	}{
		{"rule", scores.NewListRuleScoresCommand(nil), "rule ID must not be empty"},
		{"generation", scores.NewListScoresCommand(nil), "generation ID must not be empty"},
	}
...
func TestListScoresCommands_TrimIDBeforeRequest(t *testing.T) {
	cases := []struct {
		name     string
		cmd      *cobra.Command
		arg      string
		...
		{"rule", scores.NewListRuleScoresCommand(nil), "  rule-1  ", "/eval/rules/rule-1/scores"},
		{"generation", scores.NewListScoresCommand(nil), "  gen-1  ", "/query/generations/gen-1/scores"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			home := testutils.SandboxConfigEnv(t)
			...
			tc.cmd.SetOut(&bytes.Buffer{})
			tc.cmd.SetErr(&bytes.Buffer{})
			tc.cmd.SetArgs([]string{tc.arg})
			require.NoError(t, tc.cmd.ExecuteContext(context.Background()))
...
func TestNewListScoresCommand_NegativeLimit(t *testing.T) {
	cmd := scores.NewListScoresCommand(nil)
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"gen-1", "--limit", "-1"})
...
func TestNewListScoresCommand_TruncationWarning(t *testing.T) {
	assertTruncationWarning(t, scores.NewListScoresCommand(nil), []string{"gen-1", "--limit", "2", "-o", "json"}, false)
}
