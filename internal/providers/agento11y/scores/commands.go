...
func (o *listOpts) setup(flags *pflag.FlagSet) {
	...
	// A client-side safety cap bounds --limit 0 (a generation's score set can be
	// large), so the standard "0 means all" binder does not apply; an explicit
	// --limit is honored verbatim, so it stays the route to more rows.
	flags.IntVar(&o.Limit, "limit", 50, fmt.Sprintf("Maximum number of scores to return. 0 returns up to a %d-row safety cap; set a higher --limit to fetch more", scoreHardCap))
}
...
func NewListScoresCommand(loader *providers.ConfigLoader) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			genID := strings.TrimSpace(args[0])
			if genID == "" {
				return errors.New("generation ID must not be empty")
			}
			...
			scores, hasMore, err := client.ListByGeneration(cmd.Context(), genID, opts.Limit)
			if err != nil {
				return err
			}
			if err := opts.IO.Encode(cmd.OutOrStdout(), scores); err != nil {
				return err
			}
			// Output stays a bare array (a shipped shape), so the truncation hint
			// is stderr-only per output.md §15.2. safetyCap is 0 here so the hint
			// offers a --limit continuation (a generation has no filters, so a
			// larger --limit is the only route to more rows).
			meta := scoreListMeta(len(scores), opts.Limit, hasMore, 0)
