...
// intendedValue, when non-empty, is carried on the load options so a legacy
// migration performed on the way still produces the value being set. apply
// performs the field-specific edit on the loaded Config immediately before it
// is written.
func mutateKeychainPolicy(
	...
	opts := loadOptions{
		layer:                       target.Type,
		writeLockHeldFor:            sourceIdentity,
		suppressPlaintextMigration:  true,
		intendedKeychainPolicyValue: intendedValue,
	}
	opts = opts.withKeychainPolicy(policy)
	cfg, source, loadErr := loadForWrite(ctx, explicitFile, fileType, opts)
