...
const (
	...
	// documents is the raw_data document search; logs mirrors the dedicated
	// `logs` leaf command elasticsearch shipped before this PR: the same
	// search, returned newest-first, with plugin-internal fields (_source,
	// sort, highlight) omitted. They share the Lucene expression, time
	// controls, and result shape, so they are one command with a mode rather
	// than two competing leaves.
...
open it in your browser after the query succeeds.`,
  ...
  gcx datasources elasticsearch query 'level:error' --since 1h --open`,
	...
	cmd.Annotations = map[string]string{
		...
		agent.AnnotationLLMHint:   `gcx datasources elasticsearch query -d UID 'app:frontend AND level:error' --since 1h --limit 20`,
	}
