...
// TypedCRUD.ListFn's limit parameter and metav1.ListOptions.Limit.
...
// that permits it. Unimplemented capabilities return errors.ErrUnsupported
// (or the unverified dry-run path for Validator). Get explicitly disables
// TypedCRUD's list fallback when the client does not implement Getter.
func newCapabilityCRUD[T ResourceNamer](client any, meta capabilityMeta) *TypedCRUD[T] {
	...
	if getter, ok := client.(Getter[T]); ok {
		...
		crud.GetFn = func(context.Context, string) (*T, error) { return nil, errors.ErrUnsupported }
	}
