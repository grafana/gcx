...
func findByNaturalKey( //nolint:nonamedreturns // Named returns document the three-value return contract.
	ctx context.Context,
	cache *naturalKeyCache,
	desc resources.Descriptor,
	localObj *unstructured.Unstructured,
) (remoteName string, resourceVersion string, found bool) {
	...
	for i := range remoteList.Items {
		...
		if ok && remoteKey == localKey {
			return remoteList.Items[i].GetName(), remoteList.Items[i].GetResourceVersion(), true
		}
	}
