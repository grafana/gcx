...
var commandAnnotations = map[string]annotation{
	...
	// -----------------------------------------------------------------------
	"gcx kg cypher":                  {Cost: "medium", Hint: "\"MATCH (s:Service) RETURN s LIMIT 10\" [--env <env>] [--namespace <ns>] [--since 1h] | read-only Cypher query; always include LIMIT for targeted lookups; omit for broad discovery"},
