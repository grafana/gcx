...
var commandAnnotations = map[string]annotation{
	...
	"gcx irm oncall escalation-policies update-position": {Cost: "small", Hint: "<id> --position <n>; zero-based. The only way to set the step order. The backend never reports a position, so the current order is not readable."},
	...
	"gcx irm oncall routes update-position":              {Cost: "small", Hint: "<id> --position <n>; zero-based, routes match top down. The only way to set the route order. The backend never reports a position, so the current order is not readable."},
