...
var commandAnnotations = map[string]annotation{
	...
	// -----------------------------------------------------------------------
	"gcx kg entities cypher":         {Cost: "medium", Hint: "\"MATCH (s:Service) RETURN s LIMIT 10\" [--since 1h] | read-only Cypher query; always include LIMIT for targeted lookups; omit for broad discovery"},
