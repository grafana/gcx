...
var commandAnnotations = map[string]annotation{
	...
	"gcx irm oncall escalation-policies update-position": {Cost: "small", Hint: "<id> --position <n>; zero-based. The only way to set the order. The backend never reports a position, so the current order is not readable."},
