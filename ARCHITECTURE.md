...
mode, and an auto-discovered local file cannot set that policy. A missing native
keychain may permit a warned plaintext fallback for a brand-new credential; a
reachable but locked or interaction-disabled keychain fails closed with
`Keychain locked`.
