...
- **Skill lifecycle metadata lives in `claude-plugin/skills-catalog.yaml`**: Register every bundled skill as active or deprecated. When removing its content, retain its entry as retired indefinitely so old installations remain removable. See [skill lifecycle](claude-plugin/README.md#skill-lifecycle).
