...
- **Explore links are required on query commands**: Every query-class datasource leaf command must build a Grafana Explore URL in `internal/datasources/{kind}/explore.go` and expose it via `--share-link`/`--open` through `dsquery.ExploreLinkOpts` + `dsquery.EncodeAndHandleExplore`. The Explore query map must mirror the client's request body, so build both from one shared function. See Step 1c in [.claude/skills/add-datasource/SKILL.md](.claude/skills/add-datasource/SKILL.md) and `internal/datasources/clickhouse/explore.go`.
